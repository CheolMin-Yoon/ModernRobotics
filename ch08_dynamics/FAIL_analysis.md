...
## 3. 관성 파라미터 G_b FAIL (MuJoCo 비교 시)
...
| 비선형 효과 h(q,dq) | FAIL | 동일 원인 |
| 분해 검증 | FAIL | 동일 원인 |

**핵심 수정 필요 사항**:
- `params/ur5e.py`의 `Mlist`와 `Glist`를 MR 교재 Algorithm 8.2 convention에 맞게 재구성
