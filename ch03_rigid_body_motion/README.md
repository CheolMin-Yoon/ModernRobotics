...
| `modern_robotics_ch03_torch.py` | PyTorch 버전 (autograd 자동 미분 지원) |
| `modern_robotics_ch03.ipynb` | 노트북 |
| `compared_mr2pin.py` | Pinocchio 비교 (exp3/log3, exp6/log6 등) |

## PyTorch 버전

`modern_robotics_ch03_torch.py` — 모든 함수를 `torch.Tensor` 기반으로 재구현.

- 동일한 API (`RotInv`, `Vec2so3`, `MatrixExp3`, `MatrixExp6`, ...)
- autograd 자동 미분 지원: 회전 행렬의 θ에 대한 기울기 계산 가능
- numpy 원본과 교차 검증 완료 (MatrixExp3, MatrixLog3, MatrixExp6, MatrixLog, Adjoint 모두 일치)
